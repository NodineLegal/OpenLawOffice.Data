...
namespace OpenLawOffice.Data.DBOs.Activities
{
    ...
    {
        [ColumnMapping(Name = "opportunity")]
        public long? Opportunity { get; set; }

        public void BuildMappings()
        {
            Dapper.SqlMapper.SetTypeMap(typeof(ActivityRegardingOpportunity), new ColumnAttributeTypeMapper<ActivityRegardingOpportunity>());
            Mapper.CreateMap<ActivityRegardingOpportunity, Common.Models.Activities.ActivityRegardingOpportunity>()
                ...
                {
                    if (!db.Opportunity.HasValue) return null;
                    return new Common.Models.Opportunities.Opportunity()
                    {
                        Id = db.Opportunity.Value,
                        IsStub = true
                    };
                }));      
            
            Mapper.CreateMap<Common.Models.Activities.ActivityRegardingOpportunity, ActivityRegardingOpportunity>()
                .ForMember(dst => dst.Id, opt => opt.MapFrom(src => src.Id))
