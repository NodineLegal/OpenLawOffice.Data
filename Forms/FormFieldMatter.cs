...
namespace OpenLawOffice.Data.Forms
{
    ...
    public static class FormFieldMatter
    {
            ...
            bool closeConnection = true)
        {
            ...
            if (conn.Execute("INSERT INTO \"form_field_matter\" (\"id\", \"matter_id\", \"form_field_id\", \"value\", \"utc_created\", \"utc_modified\", \"created_by_user_pid\", \"modified_by_user_pid\") " +
                "VALUES (@Id, @MatterId, @FormFieldId, @Value, @UtcCreated, @UtcModified, @CreatedByUserPId, @ModifiedByUserPId)",
                dbo) > 0)
                model.Id = conn.Query<DBOs.Forms.FormFieldMatter>("SELECT currval(pg_get_serial_sequence('form_field_matter', 'id')) AS \"id\"").Single().Id;
